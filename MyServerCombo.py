...
def connectController():
	...
	time.sleep(0.050)
	print("connection has been established | ip "+add2[0]+" port "+str(add2[1]))
...
if __name__ == '__main__':
    ...
    global FrameCounter
    global conn2
    #ControllerConThread=Thread(target=connectController)
    FrameSenderThread=sendVideo()
    cmdRecThread=RecCommand()

    #ControllerConThread.start()
    if FrameCounter>3 and not(conn2 is None):
